...
export default class Cart extends PageManager {
    ...
    handleCart(prodId) {
        let myAlert = defaultModal();
        let api_string = "/cart.php?action=add&product_id="+prodId;
        //addproduct to cart using Storefront API URL
        $.post(api_string)
            .done(function (data, status, xhr) {

                //Notify user 'Item successfully added to cart'
                myAlert.open();
                myAlert.updateContent('<h1 id="modal-msg" class="modal-header-title">Product sucessfully added to cart</h1>');
                //If 'Remove All from Cart' button doesn't yet exist -> create it and set event listener
                if (!$('.deleteBtn')[0]) {
                ...
                let cartPill = $('.navUser-item--cart').find('.countPill');
                let pillCt = parseInt(cartPill.text());
                if (cartPill.hasClass('countPill--positive')) {
                    let pillCt = parseInt($('.navUser-item--cart').find('.countPill').text());
                    pillCt++;
                    cartPill.html(pillCt);
                }
                else {
    ...
    deleteCart() {
        let cartId;
        let myAlert = defaultModal();
        //Get cartId using Storefront API
        fetch('/api/storefront/cart', {
            ...
            .then(data => {
                ...
                cartId = data[0].id;
                //Delete Cart with Storefront API
